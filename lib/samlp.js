...
module.exports.parseRequest = function(req, callback) {
  ...
  getSamlRequest(samlRequest, function(err, samlRequestDom) {
    ...
    var signature = xpath.select("/*/*[local-name(.)='Signature' and namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']", samlRequestDom);
    if (signature && signature.length > 0) {
      signature = signature[0];
      signature = signature.toString();
      data.signatureWithDom = { signature, dom:samlRequestDom.toString() };
    }
...
module.exports.parseLogoutRequest = function (req, callback) {
  ...
  getSamlRequest(samlRequest, function(err, logoutRequestDom) {
    ...
    var signature = xpath.select("/*/*[local-name(.)='Signature' and namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']", logoutRequestDom);
    if (signature && signature.length > 0) {
      signature = signature[0];
      signature = signature.toString();
      data.signatureWithDom = { signature, dom:logoutRequestDom.toString() };
    }
